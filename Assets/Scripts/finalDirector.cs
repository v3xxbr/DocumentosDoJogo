...
public class finalDirector : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        spin = GameObject.FindObjectOfType<walkSpin>().gameObject;
        spin.GetComponent<Animator>().SetBool("Run", true);
    ...
    IEnumerator goToTarget(GameObject a, Vector2 target)
    {
        ...
        while (Mathf.Abs(aRbb.position.x - target.x) > 0.05f)
        {
            ...
            aRbb.velocity = new Vector2(posX * speed, 0f);

            yield return new WaitForFixedUpdate();
        }

        aRbb.position = new Vector2(target.x, aRbb.position.y);
